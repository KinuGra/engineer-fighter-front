...
import ClientGameStateManager, { GameState, PlayerState } from '../core/state/ClientGameStateManager';
...
export function useClientGameState() {
  ...
  return {
    gameState,
    addPlayer: (player: PlayerState) => ClientGameStateManager.getInstance().addPlayer(player),
    updatePlayer: (playerId: string, updates: Partial<PlayerState>) => 
      ClientGameStateManager.getInstance().updatePlayer(playerId, updates),
    removePlayer: (playerId: string) => ClientGameStateManager.getInstance().removePlayer(playerId),
